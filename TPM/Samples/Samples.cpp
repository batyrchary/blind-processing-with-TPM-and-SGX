...
Samples::Samples()
{
	...
	/*
    // The rest of this routine brings up the simulator.  This is generally not needed for a "real" TPM.
    // If the simulator is not shut down cleanly (e.g. because the test app crashed) this is called a "disorderly
	//shutdown" and the TPM goes into lockout.  The following routine will recover the TPM. This is optional - it
	//just makes debugging more pleasant.
    
	RecoverFromLockout();

    // Otherwise, power-on the TPM. Note that we power off and then power on because PowerOff cannot fail, 
	//but PowerOn fails if the TPM is already "on."
    device->PowerOff();
    device->PowerOn();

    // The following routine installs callbacks so that we can collect stats on commands executed.
    Callback1();
...
void Samples::PCR()
{
	...
	*/
    vector<TPMS_PCR_SELECTION> toReadArray { TPMS_PCR_SELECTION(TPM_ALG_ID::SHA1, 0), TPMS_PCR_SELECTION(TPM_ALG_ID::SHA256, 1)};
    ...
    vector<TPMS_PCR_SELECTION> toReadPcr0 { TPMS_PCR_SELECTION(TPM_ALG_ID::SHA1, 0) };
...
void Samples::Hash()
{
    ...
    for (auto iterator = hashAlgs.begin(); iterator != hashAlgs.end(); iterator++) 
	{
        ...
        for (int j = 0; j < 10; j++) 
		{
            // Note the syntax below. If no explicit sessions are provided then the library automatically
			//uses PWAP with the authValue contained in the handle. If you want to mix PWAP and other sessions 
			//then you can use the psuedo-PWAP session as below.
...
void Samples::GetCapability()
{
    ...
    // For simplicity, subsequent samples just get one at a time: avoiding the nested loop.
    do {
		...
		{
            cout << setw(16) << Tpm2::GetEnumString(p->alg) << ": " << Tpm2::GetEnumString(p->algProperties) << endl;
        }
...
void Samples::PrimaryKeys()
{
	...
	TPMT_PUBLIC templ(TPM_ALG_ID::SHA1,
                      ...
                      NullVec,                         // No policy
                      TPMS_RSA_PARMS( TPMT_SYM_DEF_OBJECT(TPM_ALG_ID::_NULL, 0, TPM_ALG_ID::_NULL), TPMS_SCHEME_RSASSA(TPM_ALG_ID::SHA256), 1024, 65537),TPM2B_PUBLIC_KEY_RSA(NullVec));
...
void Samples::ChildKeys()
{
    ...
    SignResponse sig = tpm.Sign(signKey, dataToSign.digest, TPMS_NULL_SIG_SCHEME(), TPMT_TK_HASHCHECK::NullTicket());
    ...
    SignResponse sigx = tpm.Sign(changedAuthHandle,dataToSign.digest, TPMS_NULL_SIG_SCHEME(),TPMT_TK_HASHCHECK::NullTicket());
    ...
    TPM_HANDLE publicKeyHandle = tpm.LoadExternal(TPMT_SENSITIVE::NullObject(),newSigningKey.outPublic,TPM_HANDLE::FromReservedHandle(TPM_RH::_NULL));
...
void Samples::PolicyORSample()
{
    ...
    tpm._Sessions(s)._ExpectError(TPM_RC::POLICY_FAIL).HMAC(hmacKeyHandle, vector<BYTE> {1, 2, 3, 4}, TPM_ALG_ID::SHA1);
...
void Samples::Attestation()
{
    ...
    // Nomrmally the verifier will have other ways of determinig the veractity of the public key
...
//    tpm.PCR_Event(TPM_HANDLE::PcrHandle(7), ByteVec { 1, 2, 3 });
    ...
    GetTimeResponse timeQuote = tpm.GetTime(tpm._AdminEndorsement, signingKey, timeNonce, TPMS_NULL_SIG_SCHEME());
    ...
    if (sigOk) {cout << "Key certification validated" << endl;}
    ...
    sigOk = pubKey.outPublic.ValidateCertifyCreation(nonce,newSigningKey.creationHash, createQuote);
    if (sigOk) 
	{
...
void Samples::DictionaryAttack()
{
    ...
    tpm.DictionaryAttackParameters(tpm._AdminLockout,newMaxTries, newRecoverTime,lockoutAuthFailRecoveryTime);
    return;
}
...
void Samples::PolicyCpHash()
{
    ...
    // PolicyCpHash restricts the actions that can be performed on a secured object to just a specific operation
	//identified by the hash of the command paramters. THis sample demonstrates how TSS.c++ can be used to obtain 
	//and use CpHashes. We demonstrate a policy that (effectively) lets anyone do a TPM Clear() operation, but no other admin tasks.
...
void Samples::PolicyWithPasswords()
{
    ...
    // commands PolicyPassword and PolicyAuthValue indicate that both must be used, as demonstrated here.
    ...
    p.Execute(tpm, sess);
    hmacSig = tpm._Sessions(sess)._ExpectError(TPM_RC::AUTH_FAIL).HMAC(hmacHandle, ByteVec { 1, 2, 3, 4 }, TPM_ALG_ID::SHA1);
...
//    tpm.PCR_Event(TPM_HANDLE::PcrHandle(pcr), ByteVec { 1, 2, 3, 4 });
    ...
    TPM_HANDLE sealedKey = tpm.Load(storagePrimary, sealedObject.outPrivate, sealedObject.outPublic);
