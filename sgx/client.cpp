...
int do_attestation(sgx_enclave_id_t eid, config_t *config, MsgIO* msgio);
int initialize_msg(MsgIO *msgio, config_t *config);
int recieveCall_sendResult(sgx_enclave_id_t eid, config_t *config, Tpm2 tpm);
char debug = 0;
char verbose = 0;
...
TPM_HANDLE MakeEndorsementKey(Tpm2 tpm)
{
	vector<BYTE> NullVec;
	...
	cout << "EK: " << ek.outPublic.ToString(false) << endl;
	return ek.handle;
}

TPM_HANDLE MakeChildSigningKey(TPM_HANDLE parentHandle, bool restricted, Tpm2 tpm)
{
	vector<BYTE> NullVec;
	...
	// Create the key
	CreateResponse newPrimary = tpm.Create(parentHandle,
		sensCreate,
		templ,
		NullVec,
		pcrSelect);

	cout << "AIK: " << newPrimary.outPublic.ToString(false) << endl;

	auto signKey = tpm.Load(parentHandle, newPrimary.outPrivate, newPrimary.outPublic);
	return signKey;
}
...
//		cout << "the complete file content is in memory" << endl;
	...
	for (auto iterator = hashAlgs.begin(); iterator != hashAlgs.end(); iterator++)
	{
		...
		cout << "TPM generated-Hash (" << Tpm2::GetEnumString(*iterator) << "):" << hashVal.result << endl;
	}
	*/

	vector<TPMS_PCR_SELECTION> toReadPcr{ TPMS_PCR_SELECTION(TPM_ALG_ID::SHA1, PCRbank) };
	auto pcrVals = tpm.PCR_Read(toReadPcr);
	cout << "SHA1 Before Extend:" << endl << pcrVals.pcrValues[0].ToString() << endl << endl;
	cout << "------------------------------" << endl;
	TPMT_HA toExtend = TPMT_HA::FromHashOfString(TPM_ALG_ID::SHA1, memblock);
	cout << "measurement of app:" << toExtend.ToString() << endl << endl;
	tpm.PCR_Extend(TPM_HANDLE::PcrHandle(PCRbank), std::vector<TPMT_HA> {toExtend});
	cout << "------------------------------" << endl;
	pcrVals = tpm.PCR_Read(toReadPcr);
...
//	memcpy(smessage, &s, 1000);
	...
	VerifySignatureResponse sigVerify = tpm._AllowErrors().VerifySignature(signKey,
		dataToSign.digest,
		*sig.signature);
...
TPM_HANDLE MakeStoragePrimary(Tpm2 tpm)
{
	vector<BYTE> NullVec;
...
TPM_HANDLE MakeChildSigningKeyForQuote(Tpm2 tpm, TPM_HANDLE parentHandle, bool restricted)
{
...
void GenerateQuote_PCR(Tpm2 tpm)
{
	...
	TPM_HANDLE signingKey = MakeChildSigningKeyForQuote(tpm, primaryKey, true);
...
//				Signing(tpm, *signingKey);
	...
	*/
	...
	//	lt = *localtime(&timeT);//Windows
	localtime_s(&lt, &timeT);//Ubuntu
	...
	while (1)
	{
		...
		case 'n':
			...
			if (!from_hexstring((unsigned char *)&config.nonce, (unsigned char *)optarg, 16))
			{
				fprintf(stderr, "nonce must be 32-byte hex string\n");
				exit(1);
			}
	...
	{
		recieveCall_sendResult(eid, &config, tpm);
...
int do_attestation(sgx_enclave_id_t eid, config_t *config, MsgIO *msgio)
{
	...
	*/
	...
	//////////////// concatenate msg0-msg1 and send them //////////////////
	//Send msg0 and msg1 concatenated together (msg0||msg1). 
	...
	//////////////////// Read msg2 /////////////////////////
	// Read msg2, it is variable length b/c it includes the revocation list at
...
//	 object msg3 is malloc'd by SGX SDK, so remember to free when finished 
	...
	status = sgx_ra_proc_msg2(ra_ctx, eid, sgx_ra_proc_msg2_trusted, sgx_ra_get_msg3_trusted, msg2, sizeof(sgx_ra_msg2_t) + msg2->sig_rl_size, &msg3, &msg3_sz);
	...
	{
		// Trusted, but client may be untrusted in the future unless it takes action.
	...
	{
		// Not Trusted, but client may be able to take action to become trusted.
	...
	///////// Get MK (Master key) and SK (Session key) from enclave ////////////////

	 // If the enclave is trusted, fetch a hash of the the MK and SK from the enclave
	 // to show proof of a shared secret with the service provider. 
	...
	//	recieveCall_sendResult(eid,  config, msgio);
...
int do_quote(sgx_enclave_id_t eid, config_t *config)
{
	...
	memset(quote, 0, sz);
	status = sgx_get_quote(&report, linkable, &config->spid, (OPT_ISSET(flags, OPT_NONCE)) ? &config->nonce : NULL, NULL, 0, (OPT_ISSET(flags, OPT_NONCE)) ? &qe_report : NULL, quote, sz);
...
#ifndef _WIN32
sgx_status_t sgx_create_enclave_search(const char *filename, const int edebug, sgx_launch_token_t *token, int *updated, sgx_enclave_id_t *eid, sgx_misc_attribute_t *attr)
{
	struct stat sb;
