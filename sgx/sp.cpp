...
int makeCall_GetResult(MsgIO *msg, IAS_Connection *ias, char *call,
	char *result, config_t *config, ra_session_t *session);

int get_sigrl(IAS_Connection *ias, int version, sgx_epid_group_id_t gid, char **sigrl, uint32_t *msg2);

int get_attestation_report(IAS_Connection *ias, int version, const char *b64quote,
	sgx_ps_sec_prop_desc_t sec_prop, ra_msg4_t *msg4, int strict_trust);
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	try
	{
		ias = new IAS_Connection((flag_prod) ? IAS_SERVER_PRODUCTION : IAS_SERVER_DEVELOPMENT, 0);
	}
	catch (...)
	{
		oops = 1;
	...
	if (config.cert_passwd_file != NULL)
	{
		...
		if (sz > 0)
		{
			...
			if (!from_file((unsigned char *)passwd, config.cert_passwd_file, &sz))
			{
				eprintf("can't load password from %s\n", config.cert_passwd_file);
				return 1;
			}
			...
			// If a key file isn't specified, assume it's bundled with the certificate.

			keyfile = (config.cert_key_file == NULL) ? config.cert_file : config.cert_key_file;
			if (debug)
				eprintf("+++ using cert key file %s\n", keyfile);
	...
	// for the TLS session. If this isn't set, then the user agent will fall back to it's default.
...
int process_msg3(MsgIO *msgio, IAS_Connection *ias, sgx_ra_msg1_t *msg1, ra_msg4_t *msg4, config_t *config, ra_session_t *session)
{
	sgx_ra_msg3_t *msg3;
	size_t blen = 0;
	size_t sz;
	...
	cmac128(session->smk, (unsigned char *)&msg3->g_a, sizeof(sgx_ra_msg3_t) - sizeof(sgx_mac_t) + quote_sz, (unsigned char *)vrfymac);
	...
	if (get_attestation_report(ias, config->apiver, b64quote, msg3->ps_sec_prop, msg4, config->strict_trust))
	{
		...
		if (verbose)
		{
			...
			// The enclave report is valid so we can trust the report data.
...
int process_msg01(MsgIO *msgio, IAS_Connection *ias, sgx_ra_msg1_t *msg1, sgx_ra_msg2_t *msg2, char **sigrl, config_t *config, ra_session_t *session)
{
	...
	/////////////////////// Read Msg0-1 /////////////////////////////
	// Read our incoming message. We're using base16 encoding/hex strings so we should end up with sizeof(msg)*2 bytes.
	...
	// "Currently, the only valid extended Intel(R) EPID group ID is zero. The server should verify this 
	// value is zero. If the Intel(R) EPID group ID is not zero, the server aborts remote attestation".
	...
	/////////////////////// Derive KDK (key derivation key) /////////////////////////////

	 // Derive the KDK from the key (Ga) in msg1 and our session key.
	 // An application would normally protect the KDK in memory to prevent trivial inspection.
	 ...
	 *
	 * Ga     = Client enclave's session key (32 bytes)
	 *
	 * Gb     = Service Provider's session key (32 bytes)
	 *
	 * SPID   = The Service Provider ID, issued by Intel to the vendor (16 bytes)
	 *
	 ...
	 *          (64 bytes)
	 *
	 * CMACsmk= AES-128-CMAC(A) (16 bytes)
	 *
	 *
	...
	if (!get_sigrl(ias, config->apiver, msg1->gid, sigrl, &msg2->sig_rl_size)) { eprintf("could not retrieve the sigrl\n");return 0; }
...
int derive_kdk(EVP_PKEY *Gb, unsigned char kdk[16], sgx_ec256_public_t g_a, config_t *config)
{
	...
	memset(cmackey, 0, 16);
	//Compute the shared secret using the peer's public key and a generated public/private key.
...
int get_attestation_report(IAS_Connection *ias, int version, const char *b64quote, sgx_ps_sec_prop_desc_t secprop, ra_msg4_t *msg4, int strict_trust)
{
	IAS_Request *req = NULL;
	...
	if (status == IAS_OK)
	{
		...
		if (!reportObj["platformInfoBlob"].IsNull())
		{
			...
			int ret = from_hexstring((unsigned char *)&msg4->platformInfoBlob, pibBuff.c_str(), pibBuff.length());
		}
		else
		{
			if (verbose)
