...
void square(size_t len, int *bufIn, int *bufOut)
{
	int ret = *bufIn;
	ret = ret * ret;

	memcpy(bufOut, &ret, len);
}
...
//  a C++ compiler. Just making the source C++ was the easiest way to deal with that.
...
sgx_status_t enclave_ra_init(sgx_ec256_public_t key, int b_pse, sgx_ra_context_t *ctx, sgx_status_t *pse_status)
{
	sgx_status_t ra_status;
	 ...
	 // If we want platform services, we must create a PSE session before calling sgx_ra_init()
	if ( b_pse ) 
	{
		int retries= PSE_RETRIES;
		do 
...
sgx_status_t enclave_ra_init_def(int b_pse, sgx_ra_context_t *ctx, sgx_status_t *pse_status)
{
 ...
 // get proof of possession of the key without exposing it to untrusted memory.
 
sgx_status_t enclave_ra_get_key_hash(sgx_status_t *get_keys_ret, sgx_ra_context_t ctx, sgx_ra_key_type_t type, sgx_sha256_hash_t *hash)
{
	...
	*get_keys_ret = sgx_ra_get_keys(ctx, type, &k);
	...
	sha_ret= sgx_sha256_msg((const uint8_t *) &k, sizeof(k), (sgx_sha256_hash_t *) hash); 
	// Sigh.
